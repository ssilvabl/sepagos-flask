...
def login():
    ...
    # Validar si no existe una sesión
    if not 'userEmail' in session:
...
def profile():
    ...
    return validateUser(protected_url='profile.html')
...
def logout():
    ...
    # Validar primero si existe una sesión activa
    ...
    # Si no existe una sesión
        ...
        # Mensaje de confirmación
        flash('Primero debe iniciar sesión', 'warning')
        # Redirigir al login
        return redirect(url_for('auth.login'))
